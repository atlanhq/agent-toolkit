...
class CustomMetadataDetectorWithSingleton:
    ...
    CUSTOM_METADATA_KEYWORDS = {
        'business metadata', 'custom metadata', 'custom metadata filters','business attributes',
        'data classification', 'data quality',
        'business context', 'metadata attributes', 'business properties',
        'custom attributes', 'business tags', 'data governance'
    }
    ...
    def detect_from_natural_language(
        self,
        query_text: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.debug(f"Starting custom metadata detection analysis for query: {query_text[:100]}...")
        ...
        if detected_keywords:
            detection_reasons.append(f"Custom metadata keywords detected: {', '.join(detected_keywords)}")
        
        # Check for data governance and quality terms
        governance_terms = self._detect_governance_terms(query_text)
        if governance_terms:
            detection_reasons.append(f"Data governance terms detected: {', '.join(governance_terms)}")
        ...
        if detected:
            ...
            try:
                ...
                logger.info(f"Provided custom metadata context with {len(context)} definitions using singleton cache")
    ...
    def _detect_governance_terms(self, text: str) -> List[str]:
        ...
        governance_terms = [
            ...
            'data lineage', 'data quality', 'data governance', 'data catalog',
            'master data', 'reference data', 'critical data', 'sensitive data',
            'public data', 'internal data', 'confidential data', 'restricted data',
            'data retention', 'data lifecycle', 'data archival', 'data purging'
        ]
