...
class SingletonCacheManager(Generic[T]):
    ...
    def __init__(self, cache_name: str, ttl_seconds: float = 300.0):
        ...
        logger.debug(
            f"Initialized singleton cache manager: {cache_name} (TTL: {ttl_seconds}s)"
        )
    ...
    def _is_cache_valid(self, current_time: float) -> bool:
        ...
        return (
            ...
            and (current_time - self._cache_timestamp) < self.ttl_seconds
        )
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        with self._data_lock:
            current_time = time.time()
            cache_size = (
                len(self._cache_data)
                if isinstance(self._cache_data, (list, dict, str))
                else 1
                if self._cache_data
                else 0
            )
            ...
            return {
                ...
                "age_seconds": current_time - self._cache_timestamp
...
def get_custom_metadata_cache_manager() -> SingletonCacheManager[List[Dict[str, Any]]]:
    ...
    """
    return SingletonCacheManager[List[Dict[str, Any]]](
        "custom_metadata_context", ttl_seconds=900.0
    )
