...
def detect_custom_metadata_trigger(query_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result["detected"]:
            logger.info(f"Custom metadata detected with reasons: {result['detection_reasons']}")
            context_count = len(result.get("context", []))
            logger.info(f"Provided {context_count} custom metadata definitions for context enrichment")
        else:
