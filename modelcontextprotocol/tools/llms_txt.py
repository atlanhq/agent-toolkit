...
class LLMSTxtManager:
    ...
    def add_source(self, name: str, llms_txt_url: str, allowed_domains: Optional[List[str]] = None):
    ...
    async def fetch_docs(self, url: str, source_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        if not allowed:
            return {
                "url": url,
                "error": f"Domain not allowed. URL must be from one of the allowed domains from configured sources.",
                "available_sources": [s.name for s in sources_to_check],
                "allowed_domains": [domain for s in sources_to_check for domain in s.allowed_domains]
            }
...
async def fetch_documentation(url: str, source_names: Optional[List[str]] = None) -> Dict[str, Any]:
    """
...
def add_doc_source(name: str, llms_txt_url: str, allowed_domains: Optional[List[str]] = None) -> Dict[str, str]:
    """
    Add a new documentation source.
