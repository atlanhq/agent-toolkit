...
def _extract_relationship_attributes(value, atlan_client, is_list=True):
    ...
    for item in items:
        ...
        if item_name is None and item_guid:
            try:
                ...
                if full_asset:
                    item_name = getattr(full_asset, 'name', None) or getattr(full_asset, 'display_name', None)
                    if not item_qualified_name:
                        item_qualified_name = getattr(full_asset, 'qualified_name', None)
        ...
        if hasattr(source_obj, 'dict'):
            try:
                ...
                if 'attributes' in relationship_obj and isinstance(relationship_obj['attributes'], dict):
                    nested_attrs = relationship_obj.pop('attributes')  # Remove and extract
    ...
    return relationship_objects if is_list else (relationship_objects[0] if relationship_objects else None)
...
def retrieve_domain(
    ...
    try:
        ...
        if domain:
            ...
            logger.debug("Using hybrid approach: domain.dict() + relationship attributes")
            ...
            if 'attributes' in domain_dict and isinstance(domain_dict['attributes'], dict):
                ...
                logger.debug(f"Flattened {len(nested_attrs)} business attributes from nested structure")
            ...
            for attr_name, attr_value in relationship_attributes.items():
                if attr_value is not None:
                    ...
                    if attr_name == 'sub_domains':
                        domain_dict[attr_name] = _extract_relationship_attributes(attr_value, atlan_client, is_list=True)
                    elif attr_name == 'parent_domain':
                        domain_dict[attr_name] = _extract_relationship_attributes(attr_value, atlan_client, is_list=False)
            ...
            logger.debug(f"Retrieved domain with {len(domain_dict)} attributes (hybrid approach with flattening)")
